...
module.exports = (db) => {
  ...
  router.get('/milestone', async (req, res) => {
    try {
      ...
      if (userId) {
        text = `SELECT id, user_id, goal_id, milestone, deadline, completed, completed_at FROM milestones WHERE user_id = ${userId}`;
      } else {
        text =
          'SELECT id, user_id, goal_id, milestone, deadline, completed, completed_at FROM milestones';
      }
